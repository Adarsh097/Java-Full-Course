...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            String dateOfBooking = sc.next();
            int noOfRoomsBooked = sc.nextInt();
            String cabFacility = sc.next();
            double totalBill = sc.nextDouble();
            motels.add(new Motel(motelId, motelName, dateOfBooking, noOfRoomsBooked, cabFacility, totalBill));
        }

        String cf = sc.next();
        int totalRooms = totalNoOfRoomsBooked(motels, cf);
        if (totalRooms > 0) {
    ...
    public static int totalNoOfRoomsBooked(ArrayList<Motel> motels, String cabFacility) {
        int totalRooms = 0;
